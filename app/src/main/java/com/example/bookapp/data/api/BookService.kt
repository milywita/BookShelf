...
suspend fun <T> safeApiCall(tag: String = BookService.TAG, call: suspend () -> T): T {
  return try {
    ...
    throw when (e) {
      ...
      is SocketTimeoutException -> AppError.Network.ServerError(message = "Request timed out", cause = e)
      is HttpException -> AppError.Network.ServerError(
        message = "Server returned error ${e.code()} - ${e.response()?.errorBody()?.string()}",
        cause = e
      )
